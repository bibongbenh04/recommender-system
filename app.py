...
def content_based_recommendations(train_data, item_name, top_n=10):
    ...
    recommended_items_details['ImageURL'] = recommended_items_details['ImageURL'].str.split('|').str[0]
    print(recommended_items_details['ImageURL'])
    # # Ensure 'ImageURL' column exists, add a placeholder if not
    # if 'ImageURL' not in recommended_items_details.columns:
    #     recommended_items_details['ImageURL'] = 'static/img/default.png' # Or any other default image
...
def recommendations():
    if request.method == 'POST':
        ...
        try:
            nbr = int(nbr_str) if nbr_str else 10  # Default to 10 if empty
        except ValueError:
            nbr = 10  # Default to 10 if not a valid number

        content_based_rec = content_based_recommendations(train_data, prod, top_n=nbr)

        if content_based_rec.empty:
            message = "No recommendations available for this product." + (f" Product '{prod}' not found." if prod else "")
